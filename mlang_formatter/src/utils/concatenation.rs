...
use biome_rowan::{SyntaxResult, TextSize};
use mlang_syntax::{AnyMExpression, AnyMLiteralExpression, MSyntaxKind, MSyntaxToken};

/// One piece of a flattened top-level `+`-concatenation chain
pub(crate) enum ConcatenationPart {
...
/// every original token in the chain can be accounted for
/// ([FormatConcatenatedQuery] marks each one "removed" for the formatter's
/// token-tracking bookkeeping, since the reconstructed output replaces all
/// of them).
pub(crate) struct ConcatenationChain {
    pub(crate) parts: Vec<ConcatenationPart>,
    pub(crate) operators: Vec<MSyntaxToken>,
}
...
///   guarantees a 1:1 correspondence between input literal parts and output
///   [FormattedPiece::Literal]s, which is what lets
///   [FormatConcatenatedQuery] account for every original token without
///   having to work out which of several original literals a merged output
///   run came from.
pub(crate) fn flatten_concatenation_chain(
    expression: &AnyMExpression,
) -> Option<ConcatenationChain> {
...
/// either way.
pub(crate) fn hole_placeholder(index: usize) -> String {
    // `std::format!`, not `biome_formatter::format!` -- the latter is
    // pulled into scope by the `crate::prelude::*` glob import above and
    // shadows the standard string-formatting macro in this file.
    std::format!("__mlang_hole_{index}__")
}

/// A string literal token's raw (still-escaped, exactly as written in
/// source) inner content, with the surrounding quotes stripped -- matches
/// `try_format_embedded_sql`'s own extraction for the single-literal case
/// (`string_utils.rs`), so a concatenation chain with only one real literal
/// segment reconstructs byte-for-byte the same source text that path would
/// have parsed.
...
pub(crate) fn build_placeholder_source(parts: &[ConcatenationPart]) -> Option<String> {
