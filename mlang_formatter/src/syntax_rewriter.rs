...
use mlang_syntax::{
    AnyMAssignment, AnyMExpression, AnyMParenthesized, MLanguage, MLogicalExpression, MSyntaxKind,
    MSyntaxNode, MSyntaxTrivia,
};
use std::collections::BTreeSet;

pub(super) fn transform(root: MSyntaxNode) -> (MSyntaxNode, TransformSourceMap) {
    let mut rewriter = MFormatSyntaxRewriter::with_offset(root.text_range().start());
    let transformed = rewriter.transform(root);
    (transformed, rewriter.finish())
}

#[derive(Default)]
struct MFormatSyntaxRewriter {
    ...
    /// }
    /// ```
    /// The rewriter first removes any leading whitespace from the `MxTagExpression`'s leading trivia.
...
impl MFormatSyntaxRewriter {
    pub fn with_offset(offset: TextSize) -> Self {
        MFormatSyntaxRewriter {
...
impl MFormatSyntaxRewriter {
    ...
    fn visit_parenthesized(
        &mut self,
        parenthesized: AnyMParenthesized,
        ...
        let (l_paren, inner, r_paren) = match (
            ...
            (Ok(l_paren), Ok(inner), Ok(r_paren)) => {
                ...
                if inner.kind().is_bogus()
                    ...
                    || prev_token.is_some_and(|prev_token| has_skipped_comment(&prev_token.trailing_trivia()))
        ...
        match inner.first_token() {
            ...
            None => {
                let updated = match parenthesized {
                    AnyMParenthesized::MParenthesizedExpression(expression) => {
                        // SAFETY: Safe because the rewriter never rewrites an expression to a non expression.
                        expression
                            .with_expression(AnyMExpression::unwrap_cast(inner))
                            .into_syntax()
                    }
                    AnyMParenthesized::MParenthesizedAssignment(assignment) => {
                        // SAFETY: Safe because the rewriter never rewrites an assignment to a non assignment.
                        assignment
                            .with_assignment(AnyMAssignment::unwrap_cast(inner))
                            .into_syntax()
                    }
                };
    ...
    ///
    /// A parenthesized group of 3+ conditions (`a && (b && c && d)`) parses
    /// its own insides left-recursively too, so the tree is
    /// `&&(a, &&(&&(b,c), d))` -- the group's `left` (`&&(b,c)`) is itself a
    /// chain, not a single atom, so pulling up only the immediate
    /// `right.left()`/`right.right()` pair (as a single rotation would)
    /// just relocates the imbalance one level down instead of removing it.
    /// Flattening both sides down to their individual atoms first and
    /// folding them back left-associatively handles any nesting depth in
    /// one pass.
    fn visit_logical_expression(
        &mut self,
        logical: MLogicalExpression,
    ) -> VisitNodeSignal<MLanguage> {
        match (logical.left(), logical.operator_token(), logical.right()) {
            (Ok(left), Ok(operator), Ok(right)) => {
                ...
                // SAFETY: Safe because the rewriter never rewrites an expression to a non expression.
                let left = AnyMExpression::unwrap_cast(self.transform(left.into_syntax()));
                let operator = self.visit_token(operator);
                // SAFETY: Safe because the rewriter never rewrites an expression to a non expression.
                let right = AnyMExpression::unwrap_cast(self.transform(right.into_syntax()));
                ...
                let updated: AnyMExpression = if atoms.len() > 2 {
                    ...
                    for atom in atoms {
                        ...
                        acc = mlang_factory::make::m_logical_expression(acc, op, atom).into();
                    }
                    acc
                } else {
...
/// its descendants) is an `MLogicalExpression` with `operator_kind`,
/// appending the operator between each adjacent pair to `operators` -- so
/// `atoms.len() == operators.len() + 1` once both `left` and `right` of a
/// node have been flattened this way. Stops descending (treats the subtree
/// as one opaque atom) at a different operator, a non-logical expression,
/// or a logical expression with a syntax error, since none of those can be
/// safely re-associated.
fn flatten_same_operator_chain(
    expr: AnyMExpression,
    operator_kind: MSyntaxKind,
    atoms: &mut Vec<AnyMExpression>,
    operators: &mut Vec<SyntaxToken<MLanguage>>,
) {
    if let AnyMExpression::MLogicalExpression(ref logical) = expr
...
impl SyntaxRewriter for MFormatSyntaxRewriter {
    type Language = MLanguage;

    fn visit_node(&mut self, node: MSyntaxNode) -> VisitNodeSignal<Self::Language> {
        match node.kind() {
            kind if AnyMParenthesized::can_cast(kind) => {
                let parenthesized = AnyMParenthesized::unwrap_cast(node);

                self.visit_parenthesized(parenthesized)
            }
            MSyntaxKind::M_LOGICAL_EXPRESSION => {
                let logical = MLogicalExpression::unwrap_cast(node);
...
fn has_skipped_comment(trivia: &MSyntaxTrivia) -> bool {
    trivia.pieces().any(|piece| piece.is_skipped())
}

#[cfg(test)]
mod tests {
    use super::MFormatSyntaxRewriter;
    use crate::{MFormatOptions, TextRange, format_node};
    ...
    fn rebalances_logical_expressions() {
        let root = parse("a && (b && c)", MFileSource::script()).syntax();

        let transformed = MFormatSyntaxRewriter::default().transform(root.clone());
        ...
        let mut logical_expressions: Vec<_> = transformed
            .descendants()
            .filter_map(MLogicalExpression::cast)
            .collect();
    ...
    fn rebalances_a_parenthesized_group_of_three_or_more_conditions() {
        ...
        // itself a chain (`b && c`), not a single atom, so one rotation
        // just relocates the imbalance instead of removing it. The fully
        // flattened, left-recursive shape is `&&(&&(&&(a,b),c),d)`, where
        // every node's `left` accumulates the chain so far and `right` is
        // always a single atom.
        let root = parse("a && (b && c && d)", MFileSource::script()).syntax();
        let transformed = MFormatSyntaxRewriter::default().transform(root);

        assert_eq!(&transformed.text().to_string(), "a && b && c && d");
    ...
    fn single_parentheses() {
        ...
        let identifier = transformed
            .descendants()
            .find_map(MIdentifierExpression::cast)
            .unwrap();
    ...
    fn adjacent_nodes() {
        ...
        let identifiers: Vec<_> = transformed
            .descendants()
            .filter_map(MIdentifierExpression::cast)
            .collect();
    ...
    0,           0,           -1,            0,
        ...
        let array = transformed
            .descendants()
            .find_map(MArrayExpression::cast)
            .unwrap();
    ...
    fn enclosing_node() {
        ...
        let expression_statement = transformed
            .descendants()
            .find_map(MExpressionStatement::cast)
            .unwrap();
...
!(
  x + y #// foo
        ...
        let unary_expressions = transformed
            .descendants()
            .filter_map(MUnaryExpression::cast)
            .collect::<Vec<_>>();
  ...
  x + y #// foo
    ...
    fn source_map_test(input: &str) -> (MSyntaxNode, TransformSourceMap) {
        let tree = parse(input, MFileSource::script()).syntax();

        let mut rewriter = MFormatSyntaxRewriter::default();
    ...
    fn mappings() {
        ...
        let formatted =
            format_node(MFormatOptions::new(MFileSource::default()), &transformed).unwrap();
