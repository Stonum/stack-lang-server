...
перем результат = запрос.установитьПараметры(
   ...
   извлечь(массив, массив[индекс1, индекс2, индекс3, "месяц"], 00.00.00),
   индекс3
);
"#
    );
...
перем результат = запрос.установитьПараметры(
   индекс, дата1, дата2,
   массив[индекс1, индекс2, индекс3, "дата"],
   массив[индекс1, индекс2, индекс3, "месяц"],
   индекс3
);
"#
    );
...
#[test]
fn pretty_format_call_with_no_break() {
    assert_fmt!(
        r#"#
ВыполнитьФункцию(индекс, дата1, дата2, массив[индекс1, индекс2, индекс3, "дата"], 1);
"#
    );

    assert_fmt!(
        r#"#
ВыполнитьФункцию(индекс, дата1, дата2, массив[индекс1, индекс2, индекс3, "дата"]);
"#
    );
}

#[test]
fn pretty_format_string_with_call() {
    assert_fmt!(
        r#"#
Ошибка("Сумма платежей равна {}, что {} суммы строки выписки".форматировать(
   суммаПлатежей,
   суммаПлатежей > строка.Сумма ? "больше" : "меньше"
));
"#
    );
    ...
    assert_fmt!(
        r#"#
ВыполнитьФункцию(
   "boxId={}&messageId={}&entityId={}&injectEntityContent={}"
      .replace("{}", "null")
      .split("=")
      .iterator()
      .join("===========")
...
#[test]
fn pretty_format_any_object_with_call() {
    assert_fmt!(
        r#"#
ВыполнитьФункцию(
   переменнаяСтрокаОченьДлинноеИмя
      .replace("{}", "null")
      .split("=")
      .iterator()
      .join("===========")
...
#[test]
fn pretty_format_query_inner_calls() {
    assert_fmt!(
        r#"#
перем результат = запрос.установитьПараметры(
   переменная1, переменная2, переменная3,
   ВызовФункцииВнутриВызова(
      переменная1, переменная2, переменная3, переменная4, переменная5, переменная6,
      переменная7, переменная8
   ),
   переменная4
);
"#
