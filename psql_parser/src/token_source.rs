...
pub struct PsqlTokenSource<'l> {
    lexer: BufferedLexer<PsqlSyntaxKind, PsqlLexer<'l>>,
...
pub(crate) type PsqlTokenSourceCheckpoint = TokenSourceCheckpoint<PsqlSyntaxKind>;

impl<'l> PsqlTokenSource<'l> {
    /// Creates a new token source.
    pub(crate) fn new(lexer: BufferedLexer<PsqlSyntaxKind, PsqlLexer<'l>>) -> PsqlTokenSource<'l> {
        PsqlTokenSource {
            lexer,
            trivia_list: vec![],
        }
    ...
    #[inline]
    fn next_non_trivia_token(&mut self, context: PsqlLexContext, first_token: bool) {
    ...
    pub fn re_lex(&mut self, mode: PsqlReLexContext) -> PsqlSyntaxKind {
        self.lexer.re_lex(mode)
    }

    /// Creates a checkpoint to which it can later return using [Self::rewind].
    pub fn checkpoint(&self) -> PsqlTokenSourceCheckpoint {
        PsqlTokenSourceCheckpoint {
    ...
    pub fn rewind(&mut self, checkpoint: PsqlTokenSourceCheckpoint) {
...
impl<'source> TokenSource for PsqlTokenSource<'source> {
    type Kind = PsqlSyntaxKind;

    /// Returns the kind of the current non-trivia token
    #[inline(always)]
    fn current(&self) -> PsqlSyntaxKind {
        self.lexer.current()
    }
    ...
    #[inline(always)]
    fn bump(&mut self) {
        self.bump_with_context(PsqlLexContext)
    }

    fn skip_as_trivia(&mut self) {
        self.skip_as_trivia_with_context(PsqlLexContext)
    }
...
impl BumpWithContext for PsqlTokenSource<'_> {
    type Context = PsqlLexContext;
    ...
    fn skip_as_trivia_with_context(&mut self, context: PsqlLexContext) {
        if self.current() != EOF {
...
impl<'source> TokenSourceWithBufferedLexer<PsqlLexer<'source>> for PsqlTokenSource<'source> {
    fn lexer(&mut self) -> &mut BufferedLexer<PsqlSyntaxKind, PsqlLexer<'source>> {
        &mut self.lexer
    }
}
