...
fn test_table_star_as_call_argument() {
    let res = parse("select count(t.*) from t", PsqlFileSource::script());
...
fn test_table_star_alongside_other_call_arguments() {
    let res = parse("select f(t.*, 1) from t", PsqlFileSource::script());
...
fn test_schema_qualified_table_star_as_call_argument() {
    let res = parse("select count(s.t.*) from s.t", PsqlFileSource::script());
...
fn test_table_star_still_works_in_select_item_position() {
    ...
    // parsing with.
    let res = parse("select t.*, t.a from t", PsqlFileSource::script());
...
fn test_bare_star_call_argument_still_works() {
    ...
    // different node (`PsqlStar`, no table qualifier) and must be
    // unaffected.
