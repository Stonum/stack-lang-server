...
fn test_substring_from_position() {
    let res = parse("select substring(str from 3)", PsqlFileSource::script());
...
fn test_substring_comma_form_still_parses_as_plain_call() {
    ...
    // `from`/`for` handling.
    let res = parse("select substring(str, 1, 5)", PsqlFileSource::script());
...
fn test_substring_single_argument_call_still_parses_as_plain_call() {
    let res = parse("select substring(str)", PsqlFileSource::script());
