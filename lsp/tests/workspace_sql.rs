//! End-to-end tests for `Workspace`'s public API against sql (`.sql`)
...
//! for a sql document instead of panicking -- this is exactly the class
...
async fn hover_on_a_sql_document_returns_none_not_panic() {
    ...
    let hover = workspace
        ...
        .await
        .expect("hover should not error on a sql document");
...
async fn completion_on_a_sql_document_returns_none_not_panic() {
    ...
    let completion = workspace
        ...
        .await
        .expect("completion should not error on a sql document");
...
async fn signature_help_on_a_sql_document_returns_none_not_panic() {
    ...
    let signature_help = workspace
        ...
        .await
        .expect("signature_help should not error on a sql document");
...
async fn semantic_tokens_on_a_sql_document_returns_none_not_panic() {
    ...
    let tokens = workspace
        ...
        .await
        .expect("semantic_tokens should not error on a sql document");
...
async fn format_a_sql_document_via_workspace() {
    ...
    let edits = workspace
        ...
        .await
        .expect("format should not error on a sql document")
...
async fn change_document_updates_a_sql_document_in_place() {
    ...
    workspace
        ...
        .await
        .expect("change_document should not error on a sql document");
