...
use sql_syntax::{
    AstNode, NeedsParentheses, SqlBinaryExpression, SqlFileSource, SqlInExpression,
    SqlIsNullExpression, SqlLanguage, SqlLogicalExpression, SqlUnaryExpression,
};
...
fn find_by_text<T: AstNode<Language = SqlLanguage>>(src: &str, text: &str) -> T {
    let tree = parse(src, SqlFileSource::script());
    assert!(
        !tree.has_errors(),
...
fn binary_expression_bound_tighter_than_parent_does_not_need_parens() {
    ...
    // original grouping.
    let inner = find_by_text::<SqlBinaryExpression>("select a + b * c", "b * c");
    assert!(!inner.needs_parentheses());
}

#[test]
fn binary_expression_at_the_root_does_not_need_parens() {
    // No expression parent at all (parent is `SqlWhereClause`).
    let root = find_by_text::<SqlBinaryExpression>("select a from t where a > 0", "a > 0");
...
fn logical_expression_mismatched_operator_needs_parens_or_parent() {
    ...
    let inner = find_by_text::<SqlLogicalExpression>("select a or b and c", "b and c");
    assert!(inner.needs_parentheses());
}
...
fn logical_expression_mismatched_operator_needs_parens_and_parent() {
    ...
    let inner = find_by_text::<SqlLogicalExpression>("select a and b or c", "a and b");
    assert!(inner.needs_parentheses());
}
...
fn logical_expression_same_operator_does_not_need_parens() {
    ...
    let inner = find_by_text::<SqlLogicalExpression>("select a or b or c", "a or b");
    assert!(!inner.needs_parentheses());
}
...
fn unary_same_sign_nested_needs_parens() {
    ...
    // comment starter.
    let inner = find_by_text::<SqlUnaryExpression>("select - -a", "-a");
    assert!(inner.needs_parentheses());
}
...
fn unary_not_nested_does_not_need_parens() {
    ...
    let inner = find_by_text::<SqlUnaryExpression>("select not not a", "not a");
    assert!(!inner.needs_parentheses());
}
...
fn is_null_expression_as_child_of_tighter_binary_needs_parens() {
    ...
    // original grouping.
    let inner = find_by_text::<SqlIsNullExpression>("select a is null + b", "a is null");
    assert!(inner.needs_parentheses());
}
...
fn is_null_expression_as_child_of_looser_logical_does_not_need_parens() {
    ...
    let inner = find_by_text::<SqlIsNullExpression>("select a is null and b", "a is null");
    assert!(!inner.needs_parentheses());
}
...
fn in_expression_as_child_of_tighter_binary_needs_parens() {
    ...
    let inner = find_by_text::<SqlInExpression>("select a in (1, 2) + 1", "a in (1, 2)");
    assert!(inner.needs_parentheses());
}
