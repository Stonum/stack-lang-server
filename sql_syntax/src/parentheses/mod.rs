//! sql supports parenthesizing expressions, e.g. `(a + b) * c`.
...
//! these trees differ in how many `SqlParenthesizedExpression` nodes they
//! contain. This module only implements the *decision* half of that
//! (`needs_parentheses`); the other half -- stripping redundant
//! `SqlParenthesizedExpression` nodes from the tree before formatting, so
...
/// improve readability.
pub trait NeedsParentheses: biome_rowan::AstNode<Language = crate::SqlLanguage> {
