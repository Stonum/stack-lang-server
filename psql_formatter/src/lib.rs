...
use psql_syntax::{AnyPsqlStatement, PsqlLanguage, PsqlSyntaxNode};
...
use comments::PsqlCommentStyle;
pub(crate) use context::PsqlFormatContext;
pub use context::PsqlFormatOptions;
use cst::FormatPsqlSyntaxNode;
...
pub(crate) type PsqlFormatter<'buf> = Formatter<'buf, PsqlFormatContext>;

/// Rule for formatting a Psql [AstNode].
pub(crate) trait FormatNodeRule<N>
where
    N: AstNode<Language = PsqlLanguage>,
{
    fn fmt(&self, node: &N, f: &mut PsqlFormatter) -> FormatResult<()> {
    ...
    fn fmt_node(&self, node: &N, f: &mut PsqlFormatter) -> FormatResult<()> {
    ...
    fn fmt_fields(&self, item: &N, f: &mut PsqlFormatter) -> FormatResult<()>;
    ...
    fn is_suppressed(&self, node: &N, f: &PsqlFormatter) -> bool {
    ...
    fn fmt_leading_comments(&self, node: &N, f: &mut PsqlFormatter) -> FormatResult<()> {
    ...
    fn fmt_dangling_comments(&self, node: &N, f: &mut PsqlFormatter) -> FormatResult<()> {
    ...
    fn fmt_trailing_comments(&self, node: &N, f: &mut PsqlFormatter) -> FormatResult<()> {
...
#[derive(Debug, Clone)]
pub(crate) struct PsqlFormatLanguage {
    options: PsqlFormatOptions,
}
impl PsqlFormatLanguage {
    pub fn new(options: PsqlFormatOptions) -> Self {
        Self { options }
    }
}

impl FormatLanguage for PsqlFormatLanguage {
    type SyntaxLanguage = PsqlLanguage;
    type Context = PsqlFormatContext;
    type FormatRule = FormatPsqlSyntaxNode;
    ...
    fn is_range_formatting_node(&self, node: &PsqlSyntaxNode) -> bool {
        AnyPsqlStatement::can_cast(node.kind())
    }

    fn options(&self) -> &PsqlFormatOptions {
        &self.options
    }
    ...
    fn create_context(
        ...
        let comments = Comments::from_node(root, &PsqlCommentStyle, source_map.as_ref());
        PsqlFormatContext::new(self.options, comments).with_source_map(source_map)
    }
}
...
/// level of the node based on the provided [PsqlFormatContext], which
...
pub fn format_range(
    ...
    biome_formatter::format_range(root, range, PsqlFormatLanguage::new(options))
}
...
pub fn format_node(
    options: PsqlFormatOptions,
    root: &PsqlSyntaxNode,
) -> FormatResult<Formatted<PsqlFormatContext>> {
    biome_formatter::format_node(root, PsqlFormatLanguage::new(options))
}
...
pub fn format_sub_tree(options: PsqlFormatOptions, root: &PsqlSyntaxNode) -> FormatResult<Printed> {
    biome_formatter::format_sub_tree(root, PsqlFormatLanguage::new(options))
}
