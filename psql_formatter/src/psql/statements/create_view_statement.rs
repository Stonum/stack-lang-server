...
use psql_syntax::PsqlCreateViewStatement;
use psql_syntax::PsqlCreateViewStatementFields;
#[derive(Debug, Clone, Default)]
pub(crate) struct FormatPsqlCreateViewStatement;
impl FormatNodeRule<PsqlCreateViewStatement> for FormatPsqlCreateViewStatement {
    fn fmt_fields(
        &self,
        node: &PsqlCreateViewStatement,
        f: &mut PsqlFormatter,
    ) -> FormatResult<()> {
        let PsqlCreateViewStatementFields {
            create_token,
            or_token,
            replace_token,
            view_token,
