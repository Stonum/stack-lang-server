...
use crate::syntax::{
    AnyMExpression, MAssignmentExpression, MConditionalExpression, MFileSource, MInitializerClause,
    MReturnStatement, MStaticMemberExpression, MSyntaxKind, MSyntaxNode, MSyntaxToken,
    MThrowStatement, MUnaryExpression,
};
use biome_rowan::{declare_node_union, AstNode, SyntaxResult};

impl AsFormat<MFormatContext> for MConditionalExpression {
    type Format<'a> = FormatRefWithRule<'a, MConditionalExpression, FormatMAnyConditionalRule>;

    fn format(&self) -> Self::Format<'_> {
        FormatRefWithRule::new(self, FormatMAnyConditionalRule::default())
    }
}

impl IntoFormat<MFormatContext> for MConditionalExpression {
    type Format = FormatOwnedWithRule<MConditionalExpression, FormatMAnyConditionalRule>;

    fn into_format(self) -> Self::Format {
        FormatOwnedWithRule::new(self, FormatMAnyConditionalRule::default())
    }
}

#[derive(Debug, Copy, Clone, Default)]
pub(crate) struct FormatMAnyConditionalRule;

impl FormatRule<MConditionalExpression> for FormatMAnyConditionalRule {
...
impl FormatMAnyConditionalRule {
    fn layout(
        &self,
        conditional: &MConditionalExpression,
...
/// A [MConditionalExpression] that itself or any of its parent's [MConditionalExpression] have a a [MxTagExpression]
/// as its [`test`](MConditionalExpression::test), [`consequent`](MConditionalExpression::consequent) or [`alternate`](MConditionalExpression::alternate).
///
/// Parenthesizes the `consequent` and `alternate` if it the group breaks except if the expressions are
/// * `null`
/// * `undefined`
/// * or a nested [MConditionalExpression] in the alternate branch
///
/// ```javascript
/// abcdefgh? (
///   <Element>
...
pub enum ConditionalMxChain {
    Chain,
    #[default]
    NoChain,
}

impl ConditionalMxChain {
    pub const fn is_chain(&self) -> bool {
        matches!(self, ConditionalMxChain::Chain)
    }
    pub const fn is_no_chain(&self) -> bool {
        matches!(self, ConditionalMxChain::NoChain)
    }
}

impl From<bool> for ConditionalMxChain {
    fn from(value: bool) -> Self {
        match value {
            true => ConditionalMxChain::Chain,
            false => ConditionalMxChain::NoChain,
        }
    }
}
