...
impl TailChainGroups {
    ...
    pub fn has_comments(&self, comments: &MComments) -> bool {
    ...
    pub(crate) fn is_member_call_chain(&self, comments: &MComments) -> bool {
    ...
    pub(super) fn any_except_last_will_break(&self, f: &mut MFormatter) -> FormatResult<bool> {
...
impl Format<MFormatContext> for TailChainGroups {
...
impl MemberChainGroup {
    ...
    pub(super) fn will_break(&self, f: &mut MFormatter) -> FormatResult<bool> {
    ...
    pub(super) fn has_comments(&self, comments: &MComments) -> bool {
    ...
    pub(super) fn needs_empty_line_before(&self) -> bool {
        ...
        first.map_or(false, |first| match first {
            ...
            ChainMember::ComputedMember { expression } => {
                ...
                match l_brack_token {
                    Ok(l_brack_token) => get_lines_before_token(&l_brack_token) > 1,
                    _ => false,
                }
            }
            _ => false,
...
impl Format<MFormatContext> for MemberChainGroup {
    fn fmt(&self, f: &mut Formatter<MFormatContext>) -> FormatResult<()> {
...
impl Format<MFormatContext> for FormatMemberChainGroup<'_> {
    fn fmt(&self, f: &mut Formatter<MFormatContext>) -> FormatResult<()> {
