use crate::formatter::context::{MFormatOptions, QuoteProperties};
use crate::formatter::prelude::*;

use crate::syntax::MSyntaxKind::{M_LONG_STRING_LITERAL, M_STRING_LITERAL};
use crate::syntax::{MFileSource, MSyntaxToken};
...
impl<'token> FormatLiteralStringToken<'token> {
    ...
    pub fn clean_text(&self, options: &MFormatOptions) -> CleanedStringLiteralText {
        ...
        let chosen_quote_style = match token.kind() {
            M_LONG_STRING_LITERAL => options.quote_style(),
...
impl FormatLiteralStringToken<'_> {
    ...
    fn compute_string_information(&self, chosen_quote: QuoteStyle) -> StringInformation {
        ...
        let chosen_quote_byte = chosen_quote.as_bytes();
        let alternate_quote_byte = alternate_quote.as_bytes();
...
impl<'token> LiteralStringNormaliser<'token> {
    ...
    fn normalise_text(&mut self) -> Cow<'token, str> {
        ...
        match self.token.parent_kind {
            ...
            StringLiteralParentKind::Member => self.normalise_type_member(str_info),
        }
    }
    ...
    fn can_remove_number_quotes_by_file_type(&self) -> bool {
    ...
    fn normalise_type_member(&mut self, string_information: StringInformation) -> Cow<'token, str> {
        let normalised = self.normalise_string_literal(string_information);
        let quoteless = &normalised[1..normalised.len() - 1];
...
mod tests {
    use super::*;
    use crate::factory::MSyntaxTreeBuilder;
    use crate::formatter::utils::FormatLiteralStringToken;
    use crate::syntax::MSyntaxKind::{M_STRING_LITERAL, M_STRING_LITERAL_EXPRESSION};
    use crate::syntax::{MStringLiteralExpression, MSyntaxToken};
    use biome_formatter::QuoteStyle;
    use biome_rowan::AstNode;
    use std::borrow::Cow;

    fn generate_syntax_token(input: &str) -> MSyntaxToken {
        let mut tree_builder = MSyntaxTreeBuilder::new();
        tree_builder.start_node(M_STRING_LITERAL_EXPRESSION);
        tree_builder.token(M_STRING_LITERAL, input);
        tree_builder.finish_node();

        let root = tree_builder.finish();

        MStringLiteralExpression::cast(root)
            .unwrap()
            .value_token()
            .unwrap()
    }
    ...
    impl AsToken {
        fn into_token(self, token: &MSyntaxToken) -> FormatLiteralStringToken {
            match self {
                AsToken::Directive => {
    ...
    fn string_borrowed() {
        ...
        for input in inputs {
            assert_borrowed_token(input, quote, quote_properties, AsToken::String)
        }
    }
    ...
    fn string_owned() {
        ...
        for (input, output) in inputs {
            assert_owned_token(input, output, quote, quote_properties, AsToken::String)
        }
    }
    ...
    fn directive_borrowed() {
        ...
        for input in inputs {
            assert_borrowed_token(input, quote, quote_properties, AsToken::Directive)
        }
    }
    ...
    fn directive_owned() {
        ...
        for (input, output) in inputs {
            assert_owned_token(input, output, quote, quote_properties, AsToken::Directive)
        }
    }
    ...
    fn member_borrowed() {
        ...
        for input in inputs {
            assert_borrowed_token(input, quote, quote_properties, AsToken::Member)
        }
    }
    ...
    fn member_owned() {
        ...
        for (input, output) in inputs {
            assert_owned_token(input, output, quote, quote_properties, AsToken::Member)
        }
    }
}
