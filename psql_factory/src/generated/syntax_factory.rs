    ...
    fn make_syntax(
        ...
        match kind {
            ...
            PSQL_ANY_ALL_EXPRESSION => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AnyPsqlAnyAllSource::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_ANY_ALL_EXPRESSION.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_ANY_ALL_EXPRESSION, children)
            }
            ...
            PSQL_CREATE_TRIGGER_STATEMENT => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlCallExpression::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
            ...
            PSQL_CREATE_VIEW_STATEMENT => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_CREATE_VIEW_STATEMENT.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_CREATE_VIEW_STATEMENT, children)
            }
            PSQL_CTE_DEFINITION => {
                let mut elements = (&children).into_iter();
            ...
            PSQL_DROP_FUNCTION_STATEMENT => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_DROP_FUNCTION_STATEMENT.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_DROP_FUNCTION_STATEMENT, children)
            }
            ...
            PSQL_DROP_TABLE_STATEMENT => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlTableNameList::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_DROP_TABLE_STATEMENT.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_DROP_TABLE_STATEMENT, children)
            }
            ...
            PSQL_DROP_VIEW_STATEMENT => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_DROP_VIEW_STATEMENT.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_DROP_VIEW_STATEMENT, children)
            }
            PSQL_EMPTY_STATEMENT => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_EMPTY_STATEMENT.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_EMPTY_STATEMENT, children)
            }
            PSQL_EXISTS_EXPRESSION => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlSubqueryExpression::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_EXISTS_EXPRESSION.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_EXISTS_EXPRESSION, children)
            }
            PSQL_FETCH_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<5usize> = RawNodeSlots::default();
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AnyPsqlFetchTail::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_FETCH_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_FETCH_CLAUSE, children)
            }
            PSQL_FETCH_ONLY_TAIL => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_FETCH_ONLY_TAIL.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_FETCH_ONLY_TAIL, children)
            }
            PSQL_FETCH_WITH_TIES_TAIL => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_FETCH_WITH_TIES_TAIL.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_FETCH_WITH_TIES_TAIL, children)
            }
            PSQL_FILTER_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<5usize> = RawNodeSlots::default();
            ...
            PSQL_GRANT_STATEMENT => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlGranteeList::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_GRANT_STATEMENT.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_GRANT_STATEMENT, children)
            }
            ...
            PSQL_ON_CONFLICT_CLAUSE => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AnyPsqlConflictTarget::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                slots.next_slot();
                if let Some(element) = &current_element
                    && AnyPsqlConflictAction::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
            ...
            PSQL_PARENTHESIZED_JOIN_BINDING => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_PARENTHESIZED_JOIN_BINDING.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_PARENTHESIZED_JOIN_BINDING, children)
            }
            ...
            PSQL_PRECISION_MODIFIER => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_PRECISION_MODIFIER.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_PRECISION_MODIFIER, children)
            }
            ...
            PSQL_RETURNS_CLAUSE => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AnyPsqlReturnsType::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
            ...
            PSQL_SELECT_ALL_QUANTIFIER => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_SELECT_ALL_QUANTIFIER.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_SELECT_ALL_QUANTIFIER, children)
            }
            PSQL_SELECT_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AnyPsqlSelectQuantifier::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
            ...
            PSQL_SELECT_DISTINCT_QUANTIFIER => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_SELECT_DISTINCT_QUANTIFIER.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_SELECT_DISTINCT_QUANTIFIER, children)
            }
            ...
            PSQL_SELECT_STATEMENT => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlFetchClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
            ...
            PSQL_SUBQUERY_BINDING => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AnyPsqlSubqueryBody::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
            ...
            PSQL_TABLE_STAR => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlStar::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_TABLE_STAR.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_TABLE_STAR, children)
            }
            ...
            PSQL_TIME_ZONE_MODIFIER => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_TIME_ZONE_MODIFIER.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_TIME_ZONE_MODIFIER, children)
            }
            PSQL_TRIGGER_EVENT => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
            ...
            PSQL_TYPE_NAME => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && AnyPsqlTypeModifier::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                slots.next_slot();
                if let Some(element) = &current_element
                    && AnyPsqlTypeArraySuffix::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
            ...
            PSQL_VALUES_CLAUSE => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlValuesRowList::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_VALUES_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_VALUES_CLAUSE, children)
            }
            PSQL_VALUES_ROW => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_VALUES_ROW.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_VALUES_ROW, children)
            }
            PSQL_VARYING_MODIFIER => {
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_VARYING_MODIFIER.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_VARYING_MODIFIER, children)
            }
            PSQL_VIEW_OPTION => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
            ...
            PSQL_WINDOW_FUNCTION_EXPRESSION => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlWindowSpecification::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_WINDOW_FUNCTION_EXPRESSION.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_WINDOW_FUNCTION_EXPRESSION, children)
            }
            PSQL_WINDOW_PARTITION_BY_CLAUSE => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlWindowPartitionByItemList::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_WINDOW_PARTITION_BY_CLAUSE.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_WINDOW_PARTITION_BY_CLAUSE, children)
            }
            PSQL_WINDOW_SPECIFICATION => {
                ...
                slots.next_slot();
                if let Some(element) = &current_element
                    && PsqlWindowPartitionByClause::can_cast(element.kind())
                {
                    slots.mark_present();
                    current_element = elements.next();
                }
                ...
                if current_element.is_some() {
                    return RawSyntaxNode::new(
                        PSQL_WINDOW_SPECIFICATION.to_bogus(),
                        children.into_iter().map(Some),
                    );
                }
                slots.into_node(PSQL_WINDOW_SPECIFICATION, children)
            }
            PSQL_WITH_CLAUSE => {
                let mut elements = (&children).into_iter();
                let mut slots: RawNodeSlots<3usize> = RawNodeSlots::default();
            ...
            PSQL_TABLE_NAME_LIST => Self::make_separated_list_syntax(
                kind,
                children,
                PsqlTableName::can_cast,
                T ! [,],
                false,
            ),
            ...
            PSQL_TYPE_NAME_LIST => Self::make_separated_list_syntax(
                kind,
                children,
                PsqlTypeName::can_cast,
                T ! [,],
                false,
            ),
            PSQL_VALUES_ROW_LIST => Self::make_separated_list_syntax(
                kind,
                children,
                PsqlValuesRow::can_cast,
                T ! [,],
                false,
            ),
            ...
            PSQL_WINDOW_PARTITION_BY_ITEM_LIST => Self::make_separated_list_syntax(
                kind,
                children,
                AnyPsqlExpression::can_cast,
                T ! [,],
