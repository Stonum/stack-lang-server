...
use super::syntax::MSyntaxKind::{EOF, M_ARRAY_HOLE};
use super::syntax::{MSyntaxKind, TextRange, T};
use super::MParser;
use super::{Absent, ParsedSyntax, Present};

use biome_parser::prelude::*;
use biome_parser::{parse_recovery::ParseRecoveryTokenSet, ParserProgress};
...
pub(crate) trait ParseArrayPattern<P: ParseWithDefaultPattern> {
    ...
    fn parse_array_pattern(&self, p: &mut MParser) -> ParsedSyntax {
        if !p.at(T!['[']) {
            return Absent;
        }
    ...
    fn parse_any_array_element(
        &self,
        p: &mut MParser,
        recovery: &ParseRecoveryTokenSet<MSyntaxKind>,
    ) -> ParsedSyntax {
        match p.cur() {
    ...
    fn parse_rest_pattern(&self, p: &mut MParser) -> ParsedSyntax {
        if !p.at(T![...]) {
            return Absent;
        }
...
pub(crate) trait ParseObjectPattern {
    ...
    fn object_pattern_kind() -> MSyntaxKind;
    ...
    fn expected_property_pattern_error(p: &MParser, range: TextRange) -> ParseDiagnostic;

    /// Parses the object pattern like node
    fn parse_object_pattern(&self, p: &mut MParser) -> ParsedSyntax {
        if !p.at(T!['{']) {
            return Absent;
        }
    ...
    fn parse_property_pattern(&self, p: &mut MParser) -> ParsedSyntax;

    /// Parses a rest property `{ ...a }`
    fn parse_rest_property_pattern(&self, p: &mut MParser) -> ParsedSyntax;
}
