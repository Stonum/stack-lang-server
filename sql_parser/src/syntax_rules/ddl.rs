...
/// `codegen/sql.ungram` for the option-list shape).
fn parse_create_function_statement(p: &mut SqlParser) -> ParsedSyntax {
    let m = p.start();
    p.bump(T![create]);
...
fn parse_view_options(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![with]) {
        return Absent;
    }
...
fn parse_create_policy_statement(p: &mut SqlParser) -> ParsedSyntax {
    let m = p.start();
    p.bump(T![create]);
    p.bump(T![policy]);
...
fn parse_create_trigger_statement(p: &mut SqlParser) -> ParsedSyntax {
    let m = p.start();
    p.bump(T![create]);
    p.bump(T![trigger]);
...
fn parse_drop_trigger_statement(p: &mut SqlParser) -> ParsedSyntax {
    let m = p.start();
    p.bump(T![drop]);
    p.bump(T![trigger]);
...
fn parse_drop_policy_statement(p: &mut SqlParser) -> ParsedSyntax {
    let m = p.start();
    p.bump(T![drop]);
    p.bump(T![policy]);
...
fn parse_table_name_for_ddl(p: &mut SqlParser) -> ParsedSyntax {
    if is_at_tilde_name_start(p) {
...
fn parse_drop_function_parameters(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T!['(']) {
        return Absent;
    }
