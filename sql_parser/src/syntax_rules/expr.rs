...
pub const EXPR_RECOVERY_SET: TokenSet<SqlSyntaxKind> =
    token_set![R_PAREN, L_PAREN, L_BRACK, R_BRACK];

pub(crate) fn parse_expression(p: &mut SqlParser) -> ParsedSyntax {
...
fn parse_binary_or_logical_expression_recursive(
    ...
    loop {
        if p.at(T![is]) {
            ...
            p.eat(T![not]);
            p.expect(T![null]);
            left = Present(m.complete(p, SQL_IS_NULL_EXPRESSION));
            continue;
        }
        ...
        if at_not_prefixed_predicate(p, T![between]) {
            ...
            parse_binary_or_logical_expression(p, OperatorPrecedence::LogicalAnd)
                .or_add_diagnostic(p, expected_expression);
            left = Present(m.complete(p, SQL_BETWEEN_EXPRESSION));
            continue;
        }
        ...
        if at_not_prefixed_predicate(p, T![in]) {
            ...
            left = Present(m.complete(p, SQL_IN_EXPRESSION));
            continue;
        }
        ...
        if let Some(like_op) = like_op {
            ...
            parse_binary_or_logical_expression(p, OperatorPrecedence::Predicate)
                .or_add_diagnostic(p, expected_expression);
            left = Present(m.complete(p, SQL_LIKE_EXPRESSION));
            continue;
        }
        ...
        let expression_kind = match op {
            T![and] | T![or] => SQL_LOGICAL_EXPRESSION,
...
fn at_not_prefixed_predicate(p: &mut SqlParser, keyword: SqlSyntaxKind) -> bool {
    p.at(keyword)
        || (p.at(T![not])
            && p.lookahead(|p| {
                p.bump(T![not]);
...
fn report_missing_left_operand(p: &mut SqlParser) {
...
fn parse_unary_expression(p: &mut SqlParser) -> ParsedSyntax {
    match p.cur() {
        T![-] | T![+] => {
            let m = p.start();
            p.bump_any();
            parse_unary_expression(p).or_add_diagnostic(p, expected_expression);
            Present(m.complete(p, SQL_UNARY_EXPRESSION))
        }
        T![not] => {
            let m = p.start();
            p.bump(T![not]);
...
fn parse_primary_expression(p: &mut SqlParser) -> ParsedSyntax {
...
fn is_at_parameter_start(p: &mut SqlParser) -> bool {
    if !p.at(T![:]) {
        return false;
    }
    let next = p.nth(1);
    matches!(next, T![ident] | SQL_NUMBER_LITERAL) || next.is_keyword()
}

fn parse_parameter_expression(p: &mut SqlParser) -> ParsedSyntax {
    ...
    let m = p.start();
    p.bump(T![:]);
    if p.at(T![ident]) || p.at(SQL_NUMBER_LITERAL) {
        p.bump_any();
    } else {
        p.bump_remap(T![ident]);
    }
    Present(m.complete(p, SQL_PARAMETER_EXPRESSION))
}

/// `array[1, 2, 3]`.
fn parse_array_expression(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![array]) {
        return Absent;
    }
...
fn parse_array_subscript_tail(p: &mut SqlParser, mut expression: ParsedSyntax) -> ParsedSyntax {
    while p.at(T!['[']) {
        ...
        p.expect(T![']']);
        expression = Present(m.complete(p, SQL_ARRAY_SUBSCRIPT_EXPRESSION));
    }
...
fn parse_cast_function_expression(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![cast]) {
        return Absent;
    }
...
fn parse_type_modifier(p: &mut SqlParser) -> ParsedSyntax {
    if p.at(T![varying]) {
        let m = p.start();
        p.bump(T![varying]);
        return Present(m.complete(p, SQL_VARYING_MODIFIER));
    }
    if p.at(T![precision]) {
        let m = p.start();
        p.bump(T![precision]);
        return Present(m.complete(p, SQL_PRECISION_MODIFIER));
    }
    if p.at(T![with]) || p.at(T![without]) {
        ...
        p.expect(T![time]);
        p.expect(T![zone]);
        return Present(m.complete(p, SQL_TIME_ZONE_MODIFIER));
    }
    Absent
}

fn parse_type_arguments(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T!['(']) {
        return Absent;
    }
    ...
    let m = p.start();
    p.bump(T!['(']);
    SqlTypeArgumentList.parse_list(p);
    p.expect(T![')']);
    Present(m.complete(p, SQL_TYPE_ARGUMENTS))
}

fn parse_type_array_suffix(p: &mut SqlParser) -> ParsedSyntax {
    if p.at(T!['[']) {
        let m = p.start();
        p.bump(T!['[']);
        p.expect(T![']']);
...
struct SqlTypeArgumentList;

impl ParseSeparatedList for SqlTypeArgumentList {
    type Kind = SqlSyntaxKind;
    type Parser<'source> = SqlParser<'source>;
    const LIST_KIND: Self::Kind = SQL_TYPE_ARGUMENT_LIST;
...
fn parse_case_expression(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![case]) {
        return Absent;
    }
    ...
    Present(m.complete(p, SQL_CASE_EXPRESSION))
}

fn parse_case_when_clause_list(p: &mut SqlParser) -> CompletedMarker {
    let m = p.start();
    while p.at(T![when]) {
        let _ = parse_case_when_clause(p);
    }
    m.complete(p, SQL_CASE_WHEN_CLAUSE_LIST)
}

fn parse_case_when_clause(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![when]) {
        return Absent;
    }
    ...
    p.expect(T![then]);
    parse_expression(p).or_add_diagnostic(p, expected_expression);
    Present(m.complete(p, SQL_CASE_WHEN_CLAUSE))
}

fn parse_case_else_clause(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![else]) {
        return Absent;
    }
    ...
    p.bump(T![else]);
    parse_expression(p).or_add_diagnostic(p, expected_expression);
    Present(m.complete(p, SQL_CASE_ELSE_CLAUSE))
}

/// A bare `*`, mainly meant for call arguments like `count(*)`. The grammar
/// allows it anywhere an expression is expected (see `sql.ungram`'s note
/// that ambiguities/precedence are out of scope), matching how `SqlName`
/// is similarly reachable from `AnySqlExpression` without every position
/// making semantic sense.
fn parse_star(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![*]) {
        return Absent;
    }
    let m = p.start();
    p.bump(T![*]);
    Present(m.complete(p, SQL_STAR))
}
...
fn parse_ident_expression(p: &mut SqlParser) -> ParsedSyntax {
...
pub(crate) fn parse_call_expression(p: &mut SqlParser, segment_count: usize) -> ParsedSyntax {
    let m = p.start();
...
fn parse_call_expression_body(p: &mut SqlParser, m: Marker) -> ParsedSyntax {
    p.expect(T!['(']);
    SqlExpressionList.parse_list(p);
    p.expect(T![')']);
    let _ = parse_filter_clause(p);

    Present(m.complete(p, SQL_CALL_EXPRESSION))
}
...
fn parse_keyword_as_call_expression(p: &mut SqlParser) -> ParsedSyntax {
    let m = p.start();
    // `name` is `AnySqlName`, a node (`SqlName | SqlTildeName`), not a
    // bare token -- the remapped ident still needs wrapping in its own
    // `SqlName` marker, same as [parse_name] does.
    let name_m = p.start();
    p.bump_remap(T![ident]);
    name_m.complete(p, SQL_NAME);
...
fn parse_filter_clause(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![filter]) {
        return Absent;
    }
...
/// `SqlCallExpression`.
fn is_at_substring_from_form(p: &mut SqlParser) -> bool {
    if !(p.at(T![ident])
...
/// input.
fn parse_substring_expression(p: &mut SqlParser) -> ParsedSyntax {
    let m = p.start();
...
fn parse_window_function_tail(p: &mut SqlParser, call: ParsedSyntax) -> ParsedSyntax {
    ...
    Present(m.complete(p, SQL_WINDOW_FUNCTION_EXPRESSION))
}

fn parse_window_specification(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T!['(']) {
        return Absent;
    }
    ...
    p.expect(T![')']);
    Present(m.complete(p, SQL_WINDOW_SPECIFICATION))
}

fn parse_window_partition_by_clause(p: &mut SqlParser) -> ParsedSyntax {
    ...
    SqlWindowPartitionByItemList.parse_list(p);
    Present(m.complete(p, SQL_WINDOW_PARTITION_BY_CLAUSE))
}

struct SqlWindowPartitionByItemList;

impl ParseSeparatedList for SqlWindowPartitionByItemList {
    type Kind = SqlSyntaxKind;
    type Parser<'source> = SqlParser<'source>;
    const LIST_KIND: Self::Kind = SQL_WINDOW_PARTITION_BY_ITEM_LIST;
...
fn parse_literal_expression(p: &mut SqlParser) -> ParsedSyntax {
    let literal_kind = match p.cur() {
        SQL_NUMBER_LITERAL => SQL_NUMBER_LITERAL_EXPRESSION,
        SQL_STRING_LITERAL => SQL_STRING_LITERAL_EXPRESSION,
        T![true] | T![false] => SQL_BOOLEAN_LITERAL_EXPRESSION,
        T![null] => SQL_NULL_LITERAL_EXPRESSION,
        _ => return Absent,
    };
...
/// `SqlNumberLiteralExpression` specifically (e.g. type arguments like
/// `numeric(10, 2)`) rather than any general expression.
pub(crate) fn parse_number_literal_expression(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(SQL_NUMBER_LITERAL) {
        return Absent;
    }
    let m = p.start();
    p.bump(SQL_NUMBER_LITERAL);
    Present(m.complete(p, SQL_NUMBER_LITERAL_EXPRESSION))
}

/// A bare string literal, used where the grammar requires a
/// `SqlStringLiteralExpression` specifically -- e.g. a `CREATE FUNCTION`/
/// `PROCEDURE` body, which covers both `'...'` and dollar-quoted
/// (`$$...$$`/`$tag$...$tag$`) spellings (both lex as `SQL_STRING_LITERAL`)
/// rather than any general expression.
pub(crate) fn parse_string_literal_expression(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(SQL_STRING_LITERAL) {
        return Absent;
    }
    let m = p.start();
...
pub(crate) fn parse_limit_offset_value(p: &mut SqlParser) -> ParsedSyntax {
...
fn parse_parenthesized_expression(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T!['(']) {
        return Absent;
    }
    ...
    p.expect(T![')']);
    Present(m.complete(p, SQL_PARENTHESIZED_EXPRESSION))
}
...
pub(crate) fn at_subquery(p: &mut SqlParser) -> bool {
    p.at(T!['('])
        && p.lookahead(|p| {
            p.bump(T!['(']);
...
pub(crate) fn parse_subquery_expression(p: &mut SqlParser) -> ParsedSyntax {
    if !at_subquery(p) {
        return Absent;
    }
    ...
    p.expect(T![')']);
    Present(m.complete(p, SQL_SUBQUERY_EXPRESSION))
}
...
/// own, so it directly produces the right `AnySqlAnyAllSource` variant.
fn parse_any_all_expression(p: &mut SqlParser) -> ParsedSyntax {
    ...
    Present(m.complete(p, SQL_ANY_ALL_EXPRESSION))
}
...
fn parse_exists_expression(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![exists]) {
        return Absent;
    }
...
fn parse_in_source(p: &mut SqlParser) -> ParsedSyntax {
    ...
    SqlExpressionList.parse_list(p);
    p.expect(T![')']);
    Present(m.complete(p, SQL_IN_VALUE_LIST))
}
...
fn parse_col_reference(p: &mut SqlParser) -> ParsedSyntax {
    ...
    if total_segments <= 1 {
        let m = p.start();
        parse_name(p).unwrap();
        return Present(m.complete(p, SQL_COL_REFERENCE));
    }
    ...
    Present(table_col_reference.complete(p, SQL_TABLE_COL_REFERENCE))
}
...
/// check risks.
pub(crate) fn is_at_table_star(p: &mut SqlParser) -> bool {
    if !is_at_name_segment_start(p) {
        return false;
    }
    p.lookahead(|p| {
...
pub(crate) fn parse_table_star(p: &mut SqlParser) -> ParsedSyntax {
    if !is_at_table_star(p) {
        return Absent;
    }
    ...
    let star = p.start();
    p.bump(T![*]);
    star.complete(p, SQL_STAR);
    Present(m.complete(p, SQL_TABLE_STAR))
}
...
fn parse_tilde_reference_or_call(p: &mut SqlParser) -> ParsedSyntax {
...
pub(crate) fn parse_table_name(p: &mut SqlParser, segment_count: usize) -> CompletedMarker {
    let table_name = p.start();
    parse_shema_qualifier(p, segment_count.saturating_sub(1));
    parse_any_name(p).or_add_diagnostic(p, expected_identifier);
    table_name.complete(p, SQL_TABLE_NAME)
}
...
pub(crate) fn parse_shema_qualifier(p: &mut SqlParser, qualifier_count: usize) {
    match qualifier_count {
        ...
        1 => {
            ...
            parse_name(p).unwrap();
            p.bump(T![.]);
            schema_name.complete(p, SQL_SHEMA_NAME);
        }
        _ => {
            let schema_name = p.start();
            ...
            parse_name(p).unwrap();
            p.bump(T![.]);
            database_name.complete(p, SQL_DATA_BASE_NAME);
...
pub(crate) fn count_dotted_name_segments(p: &mut SqlParser) -> usize {
    p.lookahead(|p| {
        let mut count = 0;
...
///
/// `SqlName`'s single child slot is declared in the grammar as a plain
...
/// `SQL_NAME(IDENT)`.
pub(crate) fn parse_name(p: &mut SqlParser) -> ParsedSyntax {
...
pub(crate) fn is_at_name_start(p: &mut SqlParser) -> bool {
...
pub(crate) fn is_at_bracket_name_start(p: &mut SqlParser) -> bool {
    ...
    p.lookahead(|p| p.re_lex(SqlReLexContext::BracketName) == IDENT)
}
...
pub(crate) fn is_at_name_segment_start(p: &mut SqlParser) -> bool {
...
pub(crate) fn bump_name_segment(p: &mut SqlParser) {
    if p.at(T![ident]) {
        p.bump(T![ident]);
    } else {
        p.re_lex(SqlReLexContext::BracketName);
        p.bump(IDENT);
    }
}
...
pub(crate) fn is_at_tilde_name_start(p: &mut SqlParser) -> bool {
    ...
    p.lookahead(|p| p.re_lex(SqlReLexContext::TildeName) == SQL_TILDE_NAME_LITERAL)
}

/// A `SqlName` (`ident`) or, in the mlang dialect, a `~name~`/`~$name~`
/// tilde name -- used specifically in table/function-name position, never
/// for columns/aliases/CTEs (see `SqlReLexContext::TildeName`).
pub(crate) fn parse_any_name(p: &mut SqlParser) -> ParsedSyntax {
    if is_at_tilde_name_start(p) {
        let kind = p.re_lex(SqlReLexContext::TildeName);
        if kind == SQL_TILDE_NAME_LITERAL {
            let m = p.start();
            p.bump(SQL_TILDE_NAME_LITERAL);
            return Present(m.complete(p, SQL_TILDE_NAME));
        }
    }
...
pub(crate) fn parse_alias(p: &mut SqlParser) {
...
fn parse_alias_column_list(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T!['(']) {
        return Absent;
    }
    ...
    let m = p.start();
    p.bump(T!['(']);
    SqlAliasColumnDefinitionList.parse_list(p);
    p.expect(T![')']);
    Present(m.complete(p, SQL_ALIAS_COLUMN_LIST))
}

struct SqlAliasColumnDefinitionList;

impl ParseSeparatedList for SqlAliasColumnDefinitionList {
    type Kind = SqlSyntaxKind;
    type Parser<'source> = SqlParser<'source>;
    const LIST_KIND: Self::Kind = SQL_ALIAS_COLUMN_DEFINITION_LIST;
...
fn is_at_alias_column_name_start(p: &mut SqlParser) -> bool {
...
fn parse_alias_column_definition(p: &mut SqlParser) -> ParsedSyntax {
    ...
    Present(m.complete(p, SQL_ALIAS_COLUMN_DEFINITION))
}

/// A parenthesized, comma-separated expression list with no trailing comma,
/// e.g. function call arguments or `VALUES (...)` items.
pub(crate) struct SqlExpressionList;

impl ParseSeparatedList for SqlExpressionList {
    type Kind = SqlSyntaxKind;
    type Parser<'source> = SqlParser<'source>;
    const LIST_KIND: Self::Kind = SQL_EXPRESSION_LIST;
...
/// aliases.
pub(crate) fn parse_column_name_list(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T!['(']) {
        return Absent;
    }
    ...
    let m = p.start();
    p.bump(T!['(']);
    SqlColumnNameList.parse_list(p);
    p.expect(T![')']);
    Present(m.complete(p, SQL_COLUMN_LIST))
}

struct SqlColumnNameList;

impl ParseSeparatedList for SqlColumnNameList {
    type Kind = SqlSyntaxKind;
    type Parser<'source> = SqlParser<'source>;
