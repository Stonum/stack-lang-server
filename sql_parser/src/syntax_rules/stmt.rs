...
pub const STMT_RECOVERY_SET: TokenSet<SqlSyntaxKind> = token_set![
    T![;],
    T![with],
    T![select],
    T![values],
...
pub(crate) fn parse_statements(p: &mut SqlParser, statement_list: Marker) {
    ...
    while !p.at(EOF) {
        ...
        if parse_statement(p)
            .or_recover_with_token_set(
                p,
                &ParseRecoveryTokenSet::new(SQL_BOGUS_STATEMENT, STMT_RECOVERY_SET),
                expected_statement,
            )
            .is_err()
        {
    ...
    statement_list.complete(p, SQL_STATEMENT_LIST);
}

pub(crate) fn parse_statement(p: &mut SqlParser) -> ParsedSyntax {
    // A stray `;` (e.g. the second `;` in `select 1;; select 2`) is a
    // harmless empty statement, not an error -- most SQL implementations
    // treat it as a no-op. Handled as a real `AnySqlStatement` alternative
    // (`SqlEmptyStatement`) rather than skipped ad hoc: `;` is both what
    ...
    if p.at(T![;]) {
        let m = p.start();
        p.bump(T![;]);
        return Present(m.complete(p, SQL_EMPTY_STATEMENT));
    }
...
fn parse_with_prefixed_statement(p: &mut SqlParser) -> ParsedSyntax {
    let m = p.start();
    ...
    match p.cur() {
        ...
        _ => {
            ...
            p.error(err);
            Present(m.complete(p, SQL_BOGUS_STATEMENT))
        }
    }
}
