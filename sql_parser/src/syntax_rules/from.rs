...
pub(crate) fn parse_from_clause(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T![from]) {
        return Absent;
    }
    ...
    Present(m.complete(p, SQL_FROM_CLAUSE))
}
...
pub(crate) struct SqlFromItemList;

impl ParseSeparatedList for SqlFromItemList {
    type Kind = SqlSyntaxKind;
    type Parser<'source> = SqlParser<'source>;
    const LIST_KIND: Self::Kind = SQL_FROM_ITEM_LIST;
...
fn parse_from_item(p: &mut SqlParser) -> ParsedSyntax {
    ...
    Present(m.complete(p, SQL_FROM_ITEM))
}
...
fn is_at_from_expression_start(p: &mut SqlParser) -> bool {
...
fn parse_join_clause_list(p: &mut SqlParser) -> CompletedMarker {
    let m = p.start();
    while is_at_join_clause(p) {
        let _ = parse_join_clause(p);
    }
    m.complete(p, SQL_JOIN_CLAUSE_LIST)
}

fn is_at_join_clause(p: &mut SqlParser) -> bool {
    p.at(T![join])
        || p.at(T![inner])
        || p.at(T![left])
        || p.at(T![right])
...
fn parse_join_clause(p: &mut SqlParser) -> ParsedSyntax {
...
pub(crate) fn parse_from_expression(p: &mut SqlParser) -> ParsedSyntax {
...
/// `DELETE FROM`, `UPDATE`).
pub(crate) fn parse_table_binding(p: &mut SqlParser) -> ParsedSyntax {
    if is_at_tilde_name_start(p) {
...
fn build_table_binding(p: &mut SqlParser, segment_count: usize) -> ParsedSyntax {
    let m = p.start();
    parse_table_name(p, segment_count);
    parse_alias(p);
    Present(m.complete(p, SQL_TABLE_BINDING))
}

fn parse_function_binding(p: &mut SqlParser, segment_count: usize) -> ParsedSyntax {
    let m = p.start();
    parse_function_binding_body(p, m, segment_count)
}
...
fn parse_function_binding_body(p: &mut SqlParser, m: Marker, segment_count: usize) -> ParsedSyntax {
    ...
    parse_alias(p);
    Present(m.complete(p, SQL_FUNCTION_BINDING))
}

/// A subquery used as a table source, e.g. `(select a from t) as sub`.
fn parse_subquery_binding(p: &mut SqlParser) -> ParsedSyntax {
    if !p.at(T!['(']) {
        return Absent;
    }
...
fn parse_subquery_binding_body(p: &mut SqlParser, m: Marker) -> ParsedSyntax {
    ...
    p.expect(T![')']);
    parse_alias(p);
    Present(m.complete(p, SQL_SUBQUERY_BINDING))
}
...
fn is_at_parenthesized_join_start(p: &mut SqlParser) -> bool {
    p.at(T!['(']) && {
        p.lookahead(|p| {
            p.bump(T!['(']);
...
/// nestable since `source` is [AnySqlFromExpression] itself, which
/// includes this node kind.
fn parse_parenthesized_join_binding(p: &mut SqlParser) -> ParsedSyntax {
    let m = p.start();
    p.bump(T!['(']);
    parse_from_expression(p).or_add_diagnostic(p, expected_from_expression);
    parse_join_clause_list(p);
    p.expect(T![')']);
    parse_alias(p);
    Present(m.complete(p, SQL_PARENTHESIZED_JOIN_BINDING))
}
