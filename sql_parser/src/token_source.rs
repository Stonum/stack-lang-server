...
pub struct SqlTokenSource<'l> {
    lexer: BufferedLexer<SqlSyntaxKind, SqlLexer<'l>>,
...
pub(crate) type SqlTokenSourceCheckpoint = TokenSourceCheckpoint<SqlSyntaxKind>;

impl<'l> SqlTokenSource<'l> {
    /// Creates a new token source.
    pub(crate) fn new(lexer: BufferedLexer<SqlSyntaxKind, SqlLexer<'l>>) -> SqlTokenSource<'l> {
        SqlTokenSource {
            lexer,
            trivia_list: vec![],
        }
    ...
    pub fn from_str(source: &'l str, source_type: SqlFileSource) -> SqlTokenSource<'l> {
        ...
        source.next_non_trivia_token(SqlLexContext, true);
        source
    }

    #[inline]
    fn next_non_trivia_token(&mut self, context: SqlLexContext, first_token: bool) {
    ...
    pub fn re_lex(&mut self, mode: SqlReLexContext) -> SqlSyntaxKind {
        self.lexer.re_lex(mode)
    }

    /// Creates a checkpoint to which it can later return using [Self::rewind].
    pub fn checkpoint(&self) -> SqlTokenSourceCheckpoint {
        SqlTokenSourceCheckpoint {
    ...
    pub fn rewind(&mut self, checkpoint: SqlTokenSourceCheckpoint) {
...
impl<'source> TokenSource for SqlTokenSource<'source> {
    type Kind = SqlSyntaxKind;

    /// Returns the kind of the current non-trivia token
    #[inline(always)]
    fn current(&self) -> SqlSyntaxKind {
        self.lexer.current()
    }
    ...
    #[inline(always)]
    fn bump(&mut self) {
        self.bump_with_context(SqlLexContext)
    }

    fn skip_as_trivia(&mut self) {
        self.skip_as_trivia_with_context(SqlLexContext)
    }
...
impl BumpWithContext for SqlTokenSource<'_> {
    type Context = SqlLexContext;
    ...
    fn skip_as_trivia_with_context(&mut self, context: SqlLexContext) {
        if self.current() != EOF {
...
impl<'source> TokenSourceWithBufferedLexer<SqlLexer<'source>> for SqlTokenSource<'source> {
    fn lexer(&mut self) -> &mut BufferedLexer<SqlSyntaxKind, SqlLexer<'source>> {
        &mut self.lexer
    }
}
