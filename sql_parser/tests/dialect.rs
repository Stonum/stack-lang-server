...
fn test_mlang_dialect_flag_round_trips() {
    let source = SqlFileSource::script().with_dialect(SqlDialect::Mlang);

    assert_eq!(source.dialect(), SqlDialect::Mlang);
...
fn test_hash_temp_table_name_in_insert_update_delete() {
    let dialect = || SqlFileSource::script().with_dialect(SqlDialect::Mlang);
