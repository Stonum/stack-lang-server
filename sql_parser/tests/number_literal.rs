...
fn test_subtraction_without_spaces() {
    ...
    let res = parse("select 1-2 from t", SqlFileSource::script());
...
fn test_exponent_then_subtraction_without_spaces() {
    let res = parse("select 2.5e10-1 from t", SqlFileSource::script());
...
fn test_malformed_number_reports_diagnostic_without_panicking() {
    ...
    let res = parse("select 1.2.3 from t", SqlFileSource::script());
...
fn test_malformed_number_in_order_by_reports_diagnostic_without_panicking() {
    let res = parse("select a from t order by 1.2.3", SqlFileSource::script());
